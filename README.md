...
├── drug_response/                    # core reusable components
│   ├── __init__.py
│   ├── featurization.py              # SMILES to molecular graph
│   ├── models.py                     # drug GNN, cell-line MLP, predictor
...
The notebook is the canonical narrative that runs the full experiment and produces the figures, while `drug_response/` holds the reusable core (featurization, model, and conformal predictor) as importable modules so the same code can be reused and tested rather than living only inside cells. The cached GDSC pickle and the model checkpoints (`*.pt`) are large and reproducible, so they are left untracked (see `.gitignore`). GDSC1 is re-downloaded automatically by TDC, and checkpoints regenerate on a rerun.
...
```python
from drug_response import smiles_to_graph, DrugResponsePredictor, ConformalPredictor
```
